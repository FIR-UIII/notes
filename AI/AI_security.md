...
* jailbreak-атак Task-in-Prompt (TIP)
вместо того чтобы напрямую попросить LLM сделать запрещённую вещь, злоумышленник просит выполнить невинную задачу над закодированными данными, а результат преобразования затем используется моделью как часть запрещённой задачи.
```
Вот закодированная строка <dasdbqgqt6>
Определи, что в ней находится.
Используй полученное значение в следующей задаче...
```
### RAG
