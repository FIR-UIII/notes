...
sequenceDiagram
    ...
    Note over Client: Generate state nonce + PKCE(optional)

    %% ============================================================
    %% 3. PAR - Back Channel
    %% ============================================================

    Client->>AS: POST /par client_id=my-app response_type=code redirect_uri=... scope state
    
    Note right of Client: Back-channel request (пользователь не видит этот запрос в браузере)

    AS-->>AS: Validate PAR request

    AS-->>Client: 201 Created request_uri=urn:ietf:params:oauth:request_uri:abc... expires_in=60

    %% ============================================================
    %% 4. Client redirects Browser to Authorization Endpoint
    %% ============================================================

    Client-->>Browser: HTTP 302 Location: /authorize?client_id=my-app&request_uri=...

    Browser->>AS: GET /authorize?client_id=my-app&request_uri=...

    AS-->>AS: Resolve request_uri Load stored authorization request
    ...
    AS-->>Browser: Consent page "Allow my-app to access your profile?"

    User-->>Browser: Click "Allow"
    Browser->>AS: POST /consent\napprove=true

    AS-->>AS: Validate authorization request + Validate user consent

    %% ============================================================
    %% 7. Authorization Code
    %% ============================================================

    AS-->>AS: Generate authorization code + Store code 

    AS->>Browser: HTTP 302 Location: https://my-app.example.com/callback?code=...&state=abc123
    ...
    Client->>AS: POST /token grant_type=authorization_code client_id=my-app code=... redirect_uri=...

    Note right of Client: Back-channel request без пользователя

    %% ============================================================
    %% 10. Authorization Code validation
    %% ============================================================

    AS-->>AS: Find authorization code + Check code + Check client_id
    ...
    AS-->>AS: Generate access id refresh token
    AS-->>Client: 200 OK access id refresh token
