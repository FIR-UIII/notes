...
# NUM REDO - кол-во операций бэктрекинга (перебор подходящих под условие вариантов)
...
1. OPA начинает вычислять правило allow:
   · Проверяет is_admin → input.user == "admin" → ложь → redo (возврат к выбору варианта).
2. Переходит ко второму варианту allow:
   · Проверяет has_access → оба условия (user == "bob", resource == "report") истинны → успех.

Пример трассировки с redo
# Трассировка:
1. Enter data.example.allow
2. | Enter is_admin
3. | | Fail input.user == "admin"
4. | Exit is_admin
5. | Redo data.example.allow # Возврат к выбору варианта!
6. | Enter has_access
7. | | Enter input.user == "bob"
8. | | | Success
9. | | Enter input.resource == "report"
10.| | | Success
11.| | Exit has_access
12.| Exit data.example.allow # Результат: true
