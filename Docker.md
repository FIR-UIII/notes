...
**Контейнер Docker** – это изолированное на уровне ядра ОС, запускаемое программное приложение или сервис, которое имеет свое пространство имен и cgroup (контрольные группы). Контейнер изменяемый, создается из образа, к которому применяется dockerfile (инструкция как запустить образ).
