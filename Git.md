...
### «Откатить» файл 
```
git restore <file> если 
git checkout <file> вернуть файл в исходное состояние, вкл. удаленный
```
### «Откатить» состояние лок.репо до последнего коммита с удалением данных
```
git reset --hard <commit hash>
  * commit hash если указать то локреп откатиться до указ. хеша коммита, история удалится 
  * если хеш не указывать то локреп откатиться до последнего коммита
```
### «Откатить» состояние локреп до коммита с сохранением истории
```
git revert <commit_hash> добавит новый коммит который вернёт состояние всего локреп до состояния указанного хеша с сохранением всей истории
```
